...
- Friendli reasoning models emit `reasoning_effort` via the existing `supportsReasoningEffort` / `omitReasoningEffort` compat fields. Friendli serves GLM-5.2 reasoning via the same `chat_template_kwargs.enable_thinking` toggle NVIDIA NIM Qwen uses but, unlike NIM, also accepts top-level `reasoning_effort` for the `high`/`max` ladder. The `qwen-template-false` encoder branch emits `reasoning_effort` when `!omitReasoningEffort && wireEffort !== undefined`, so NIM (strict schema, `omitReasoningEffort: true`) stays safe.
